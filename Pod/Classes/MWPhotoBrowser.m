...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    navBar.titleTextAttributes = @{ NSForegroundColorAttributeName:navBar.tintColor};
...
- (void)playButtonTapped:(id)sender {
    ...
    if (index != NSUIntegerMax) {
//        if (!_currentVideoPlayer2ViewController) {
...
//    _currentVideoPlayerViewController.moviePlayer.scalingMode = MPMovieScalingModeFill;
...
//    [self presentViewController:_currentVideoPlayerViewController animated:YES completion:nil];
    ...
    _currentVideoPlayer2ViewController = [[AVPlayerViewController alloc] init];
    // 控制器的player播放器
    _currentVideoPlayer2ViewController.player = avPlayer;
    
    // 试图的填充模式
    _currentVideoPlayer2ViewController.videoGravity = AVLayerVideoGravityResizeAspect;
    // 是否显示播放控制条
    _currentVideoPlayer2ViewController.showsPlaybackControls = YES;
    // 设置显示的Frame
    _currentVideoPlayer2ViewController.view.frame = self.view.bounds;
    _currentVideoPlayer2ViewController.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
//    // 将播放器控制器添加到当前页面控制器中
//    [self addChildViewController:_currentVideoPlayer2ViewController];
//    // view一定要添加，否则将不显示
//    [self.view addSubview:_currentVideoPlayer2ViewController.view];
    // 播放
    [_currentVideoPlayer2ViewController.player play];
    
//    [_currentVideoPlayer2ViewController willMoveToParentViewController:self];
    
    
    typeof(self) __weak weakSelf = self;
    [self presentViewController:_currentVideoPlayer2ViewController animated:YES completion:^{
//        [weakSelf setVideoLoadingIndicatorVisible:NO atPageIndex:index];
//         _currentVideoIndex = NSUIntegerMax;
        [weakSelf clear:index];
    }];
}
